...
const getValues = (item: NotificationType): NotificationDestructured => {
	const before = item.life
		? typeof item.life === "object"
			? item.life.time
			: item.life
		: VARIABLES.DEFAULT_TIME;
	const time: number | "infinite" =
		typeof before === "number" ? before / 100 : before;
	const onHoverPause: boolean =
		typeof item.life === "object" ? item.life.onHoverPause : true;
	const showClose: boolean =
		typeof item.life === "object" ? item.life.showClose : true;
